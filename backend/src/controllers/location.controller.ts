  ...
  try {
    ...
    const locationEvent = await prisma.locationEvent.create({
      data: {
        ...
        batteryLevel: batteryLevel ? parseFloat(batteryLevel) : 85,
    ...
    return res.json({ status: 'Location recorded in TimescaleDB', eventId: locationEvent.id });
  } catch (error) {
...
export const getLatestLocations = async (req: Request, res: Response) => {
  try {
    ...
    // Base Campus Coordinates: 12.9337° N, 77.6051° E
    ...
    const locations = students.map((s: any, index: number) => {
      ...
      return {
        ...
        batteryLevel: latestPing ? latestPing.batteryLevel : (65 + (index % 30)),
